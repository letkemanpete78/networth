...
public class NetworthController {
  ...
  String addItem() {
    ...
    List<String> abc = lineItems.stream().map(x -> x.getUuid()).collect(Collectors.toList());

    List<LineItem> foundItems = lineItemRepository.findByuuidIn(abc);
    if (foundItems == null || foundItems.isEmpty()) {
      // add all records
    } else {
      List<LineItem> updateItems =  lineItems.stream()
          .filter(x -> foundItems.stream().anyMatch(y -> y.getUuid().equals(x.getUuid()))).collect(
              Collectors.toList());
    }
...
//    lineItemRepository.save(lineItem);
  ...
  public String submitdata(@RequestBody String payload) {
    ...
    try {
      List<LineItem> lineItems = new ObjectMapper()
          .readValue(payload, new TypeReference<List<LineItem>>() {
          });
    } catch (JsonProcessingException e) {
      e.printStackTrace();
    }
